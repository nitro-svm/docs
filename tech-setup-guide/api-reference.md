...
* **Management HTTP endpoints**
  ...
  * **GET /ready**\
    Returns `200 OK` when the server is accepting new sessions.\
    Returns `503` when the server is draining and not accepting new sessions.
  * **GET /available-ranges**\
    Returns an array of objects describing available replay ranges:
  * `snapshotSlot`
  * `snapshotSlotUtc`
  * `bundleStartSlot`
  * `bundleStartSlotUtc`
  * `maxBundleEndSlot`
  * `maxBundleEndSlotUtc`
* **Range Validation (createBacktestSession)**
  * The server validates the requested slot range before creating a session.
    * `startSlot <= endSlot`
    * `startSlot` must exactly match a `bundleStartSlot` returned by `GET /available-ranges`
    * `endSlot` must be `<= maxBundleEndSlot` for the selected `startSlot`
  * If any of these rules are violated, the session creation request will return an error.
* **Replay Environment**
...
*   **Sequenced Responses**
    ...
    * `seqId` is a monotonically increasing sequence number.\
      Clients can pass their last received sequence when calling `attachBacktestSession` to resume the stream without replaying earlier messages.
* **Session startup**
