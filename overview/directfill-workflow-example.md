...
**What you get:** for every historical swap on your pair in the chosen slot range, the sim runs the same trade through your Hadron pool, using your price at that moment, and compares your output with the fill the trader actually got on-chain. The result is a per-trade and average bps comparison.

**What you need:**

* Hadron's program. Nothing to supply, since it's on mainnet and the sim loads it automatically.
...
You don't need to write Hadron's account layout from scratch. Copy an existing pool and edit it:

1. Fetch an existing Hadron pool's account data with `getAccountInfo`. This is your model.
...
3. Keep the owner as Hadron's program, and keep the account rent-exempt.
...
Note: if Hadron checks relationships between accounts, such as vaults being derived from the pool and mint, those have to be consistent, or swaps will fail.
...
[
  {
    "label": "hadron/my-pool/usdc-to-token",
    "mode": "ExactIn",
    "inputMint": "<USDC>",
    "outputMint": "<your token>",
    "instruction": {
      "programId": "<Hadron program ID>",
      "accounts": [
...
* Use Hadron's swap instruction in its exact account order. Copying a real Hadron swap transaction is the easiest way.
